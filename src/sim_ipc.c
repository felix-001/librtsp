// Last Update:2019-05-29 20:09:17
/**
 * @file sim_ipc.c
 * @brief  使用原始h264和aac文件模拟摄像头
 * @author felix
 * @version 0.1.00
 * @date 2019-03-20
 */
...
static void * sim_ipc_video_task( void *instance )
{
    ...
    while ( ipc->running && p_video_buf < p_end
        ...
        p_video_buf += 4;
        if ( frame_len > 10000000 ) {
            LOGE("check h264 file error, the file is not 4byte length + nal data\n");
            return NULL;
        }
